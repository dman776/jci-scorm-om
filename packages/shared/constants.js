// @ts-check
/**
 * Shared, framework-free constants. Imported by both the Node authoring
 * side and the browser runtime player, so this file must stay dependency-free
 * and use only syntax valid in both environments.
 */
...
/**
 * Per-question response state. A question can be answered yet still not
 * satisfy its requirement (numeric below min, checklist missing expected
 * items, malformed url), which is what PARTIAL represents.
 */
...
/** SCORM 2004 success_status values (only used when reportSuccess = true). */
export const SUCCESS_STATUS = Object.freeze({
  PASSED: 'passed',
  FAILED: 'failed',
  UNKNOWN: 'unknown',
});
