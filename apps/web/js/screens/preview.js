...
export function renderPreview(mount, { store }) {
  ...
  async function load() {
    ...
    try {
      ...
      // Cache-bust so the freshly-assembled runtime is loaded.
  ...
  mount.appendChild(h('div.screen-inner', [
    ...
    h('div.preview-toolbar', [
      ...
      h('span.preview-hint', 'Rating scales are resolved exactly as they will be in the package. Partially complete sections show in orange.'),
    ]),
    frameWrap,
  ]));
  setTimeout(load, 50);
