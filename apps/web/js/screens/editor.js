...
export function renderEditor(mount, { store }) {
  ...
  const rerender = () => { mount.innerHTML = ''; renderEditor(mount, { store }); };
...
function renderRail(wb, store, rerender) {
  return h('aside.section-rail', [
    h('div.rail-head', [
      ...
      h('button.btn.small', { onclick: () => { store.addSection(); selectedSectionId = wb.sections[wb.sections.length - 1].id; rerender(); } }, '+ Add'),
    ]),
    wb.sections.length === 0
      ...
      : h('ul.rail-list', wb.sections.map((s, i) => h('li.rail-item' + (s.id === selectedSectionId ? '.active' : ''), {
        ...
        h('div.rail-item-main', [
          h('span.rail-num', String(i + 1)),
          h('span.rail-name', s.title || 'Untitled'),
        ]),
        h('div.rail-item-sub', [
  ...
  return h('div.q-card', [
    ...
    h('div.grid-2', [
      h('label.field', [
        ...
        h('select', { onchange: (e) => { store.update(() => changeType(q, e.target.value)); rerender(); } }, ALL_TYPES.map((t) =>
          h('option', { value: t.value, selected: q.type === t.value }, t.label + (t.mvp ? '' : ' (post-MVP)')))),
      ]),
    ...
    q.type === 'rating' ? renderRating(q, store) : null,
  ...
  return h('div.options-block', [
    ...
    gated
      ? h('p.block-hint', expectedCount
          ? `The learner must select all ${expectedCount} option${expectedCount === 1 ? '' : 's'} marked Expected for this question to count as complete. Expected options are never visually marked for the learner.`
    ...
    h('ul.opt-list', q.options.map((o) => h('li.opt-row', [
      ...
      h('label.opt-expected' + (o.expected ? '.on' : ''), {
        title: gated
          ? 'The learner must select this option to complete the question.'
          : 'Expected has no effect on this question type.',
      }, [
        h('input', {
          type: 'checkbox', checked: !!o.expected, disabled: !gated,
    ...
    h('button.btn.small.ghost', { onclick: () => { store.update(() => q.options.push({ id: newOptionId(), label: 'New option' })); rerender(); } }, '+ Add option'),
  ]);
}

function renderRating(q, store) {
  const isNumeric = Array.isArray(q.scale) && typeof q.scale[0] === 'number';
  return h('div.rating-block', [
    h('span.field-label', 'Rating scale'),
    h('select', {
      onchange: (e) => store.update(() => { q.scale = e.target.value === 'lmh' ? ['Low', 'Medium', 'High'] : [1, 2, 3, 4, 5]; }),
    }, [
      h('option', { value: '15', selected: isNumeric }, '1 - 5 (numeric)'),
      h('option', { value: 'lmh', selected: !isNumeric }, 'Low / Medium / High'),
    ]),
  ]);
}
...
function renderNumeric(q, store, rerender) {
  ...
  const invalidRange = isNum(q.min) && isNum(q.max) && Number(q.min) > Number(q.max);
  return h('div.numeric-block', [
    ...
    h('div.grid-2', [
      h('label.field', [
        h('span.field-label', 'Minimum'),
        h('input', {
          type: 'number', value: numOrEmpty(q.min), placeholder: 'none',
          oninput: (e) => { store.update(() => setBound('min', e.target.value)); rerender(); },
        }),
      ]),
      h('label.field', [
        h('span.field-label', 'Maximum'),
        h('input', {
          type: 'number', value: numOrEmpty(q.max), placeholder: 'none',
    ...
    invalidRange ? h('p.block-error', 'Minimum is greater than maximum. This blocks export.') : null,
...
function changeType(q, type) {
  ...
  if (CHOICE_TYPES.includes(type)) {
    ...
    delete q.scale; delete q.min; delete q.max; delete q.integerOnly;
  } else if (type === 'rating') {
    q.scale = q.scale && q.scale.length ? q.scale : [1, 2, 3, 4, 5];
    delete q.options; delete q.min; delete q.max; delete q.integerOnly;
  } else if (type === 'numeric') {
    delete q.options; delete q.scale;
  } else {
    delete q.options; delete q.scale; delete q.min; delete q.max; delete q.integerOnly;
  }
}
